...
1. Add a development project using `stoker add-project -n <PROJECT_NAME> --development`. You'll be prompted to add the first tenant to your project (requires an organization name and a user). If your deployment is stopped for any reason, you can run the `add-project` command again to resume. It's normal to see some error messages in the terminal output during deployment.
