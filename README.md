...
1. Add a development project using `stoker add-project -n <PROJECT_NAME> --development`. You'll be prompted to add the first tenant to your project (requires an organization name and a user). It's normal to see some error messages in the terminal output during deployment. If your deployment is stopped for any reason, you can run the same command again to resume.
