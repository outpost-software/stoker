...
export const validateUser = async (
    ...
    return (async () => {
        ...
        const deleteUser = async () => {
            ...
            try {
                await db.collection("system_user_permissions")
                    .doc(user.uid).delete();
            } catch (error: unknown) {
                errorLogger(error);
            }
        ...
        if (!claims?.collection) {
            await deleteUser();
            return;
        }

        const permissionsSnapshot =
            await db.collection("system_user_permissions").doc(user.uid).get();
        ...
        const snapshot =
            await db.collection(claims.collection)
                .doc(permissions.Doc_ID).get();
        if (!snapshot.exists) {
            await deleteUser();
            return;
        }
