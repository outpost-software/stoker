...
export const deleteFolder = async (
    ...
    try {
        ...
        if (!(token?.collection && token?.role && token?.doc)) {
        ...
        const permissionsSnapshot =
            await db.collection("system_user_permissions").doc(user).get();
        ...
        const pathRef = getFirestorePathRef(db, collectionPath);
        ...
        const basePath = `${collectionPath.join("/")}/${id}`;
