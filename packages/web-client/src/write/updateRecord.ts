...
export const updateRecord = async (
    ...
    originalRecord =
        ...
        originalRecord ||
        (await getOne([labels.collection], docId, { noComputedFields: true, noEmbeddingFields: true }))
    ...
    addDenormalized(
        ...
        (field: CollectionField) =>
            doc(db, "system_fields", labels.collection, `${labels.collection}-${field.name}`, docId),
        (field: CollectionField, uniqueValue: string) =>
            doc(db, "system_unique", labels.collection, `Unique-${labels.collection}-${field.name}`, uniqueValue),
        (role: StokerRole) => doc(db, "system_fields", labels.collection, `${labels.collection}-${role}`, docId),
        (relationPath: string[], id: string) => doc(db, relationPath.join("/"), id),
        (field: RelationField, dependencyField: string, id: string) =>
            doc(db, "system_fields", field.collection, `${field.collection}-${dependencyField}`, id),
        (field: RelationField, role: StokerRole, id: string) =>
            doc(db, "system_fields", field.collection, `${field.collection}-${role.replaceAll(" ", "-")}`, id),
        originalRecord,
    )

    batch.update(doc(db, path.join("/"), docId), partial)
