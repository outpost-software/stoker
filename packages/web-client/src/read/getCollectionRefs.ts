...
import { getCollectionConfigModule, getCurrentUserPermissions, getSchema } from "../initializeStoker"
...
export const getCollectionRefs = (path: string[], roleGroup: RoleGroup, getAll?: boolean) => {
    ...
    const getQueries = (constraints: QueryFieldFilterConstraint[] = []): Query[] => {
        ...
        if (fullCollectionAccess) {
            queries.push(
                query(
                    collection(db, "system_fields", labels.collection, `${labels.collection}-${roleGroup.key}`),
            ...
            for (const field of dependencyAccess) {
                queries.push(
                    query(
                        collection(db, "system_fields", labels.collection, `${labels.collection}-${field.field}`),
                        where("Collection_Path_String", "==", path.join("/")),
                        ...constraints,
                    ),
                )
