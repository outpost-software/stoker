...
import { getCurrentUserPermissions, getSchema } from "../initializeStoker"
...
export const getDocumentRefs = (path: string[], docId: string, roleGroup: RoleGroup) => {
    ...
    if (fullCollectionAccess) {
        queries.push(doc(db, "system_fields", labels.collection, `${labels.collection}-${roleGroup.key}`, docId))
    } else if (dependencyAccess) {
        for (const field of dependencyAccess) {
            queries.push(doc(db, "system_fields", labels.collection, `${labels.collection}-${field.field}`, docId))
        }
    }
    return queries
}
