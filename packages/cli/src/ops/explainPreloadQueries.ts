...
export const explainPreloadQueries = async (options: any) => {
    ...
    const permissionsSnapshot = await db.collection("system_user_permissions").doc(options.id).get()
    ...
    for (const collection of preloadCollections) {
        ...
        const queries = getCollectionRefs([collection], schema, options.id, permissions).map((ref: Query) => {
