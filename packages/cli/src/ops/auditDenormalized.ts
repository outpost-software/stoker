    ...
    for (const [collectionName, collectionSchema] of Object.entries(schema.collections)) {
        ...
        for (const [groupKey, groupFields] of Object.entries(fieldAccessGroups)) {
            ...
            for (const roleGroup of roleGroups) {
                ...
                if (overlayIndexFields.length === 0) continue
                    ...
                    for (const indexField of overlayIndexFields) {
                        ...
                        if (isRelationField(indexField)) {
                            ...
                            if (!isEqual(overlayValue, collectionValue)) {
                                console.log(
                                    `${collectionName} ${overlay.id} Field Access Group ${overlayKey}: ${indexField.name} - ${JSON.stringify(overlayValue)} !== ${JSON.stringify(collectionValue)}`,
                                )
                            }
                        } else {
                        ...
                        if (lowercaseFieldNames.includes(indexField.name)) {
                            ...
                            if (!isEqual(overlayValue, collectionValue)) {
                                console.log(
                                    `${collectionName} ${overlay.id} Field Access Group ${overlayKey}: ${indexField.name}_Lowercase - ${JSON.stringify(overlayValue)} !== ${JSON.stringify(collectionValue)}`,
                                )
                            }
                        }
                    }
