...
import { setTenant } from "./project/setTenant.js"
...
import { listTenants } from "./ops/listTenants.js"
...
program
    .command("set-tenant")
    .description('select a tenant to work with. Must be used in conjunction with `export GCP_PROJECT="<PROJECT_NAME>"`')
    .action(() => {
        setTenant()
    })
...
program
    ...
    .requiredOption("-n, --name <name>", "the Google Cloud Project ID for the tenant")
    ...
    .option("-d, --development", "specifies that this will be a development tenant")
    .option("-t, --test-mode", "add the tenant in test mode")
    .action((options) => {
        addTenant(options)
    })
...
program
    ...
    .description("delete a tenant. Be careful!")
    .option("-t, --test-mode", "delete the tenant in test mode")
    .action((options) => {
...
program
    ...
    .action((options) => {
        addRecordPrompt(options.collection, options.fullAccess, options.mode).then(() => {
            process.exit(0)
        })
    })
...
program
    ...
    .description("list Stoker tenants")
    .action(() => {
        listTenants()
    })
