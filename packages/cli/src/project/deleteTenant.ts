...
export const deleteTenant = async (options: { testMode?: boolean }) => {
    ...
    if (!options.testMode) {
        ...
        projectData.tenants = projectData.tenants.filter((tenant: string) => tenant !== process.env.GCP_PROJECT)
        projectData.deleted_tenants.push(process.env.GCP_PROJECT)
        await writeFile(join(process.cwd(), "project-data.json"), JSON.stringify(projectData, null, 4))
        console.log("Tenant deleted from project data.")
    }
    ...
    if (existsSync(join(process.cwd(), ".env", `.env.${process.env.GCP_PROJECT}`))) {
        ...
        console.log("Tenant environment file deleted.")
    }
    ...
    if (existsSync(join(process.cwd(), ".migration", process.env.GCP_PROJECT))) {
        ...
        console.log("Tenant migration folder deleted.")
    }
