import { runChildProcess, initializeStoker, fetchCurrentSchema } from "@stoker-platform/node-client"
...
import { isRelationField, retryOperation } from "@stoker-platform/utils"
import { SecretManagerServiceClient } from "@google-cloud/secret-manager"
import { getFirestore } from "firebase-admin/firestore"

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const addTenant = async (options: any) => {
    ...
    if (process.env.GCP_PROJECT) {
        throw new Error("GCP_PROJECT should not be set for tenant creation.")
    }
...
STOKER_FB_EMULATOR_FUNCTIONS_PORT=${functionsPort}`
        ...
        for (const [collectionName, collection] of Object.entries(schema.collections)) {
            if (collection.auth || collection.singleton) {
                for (const field of collection.fields) {
                    if (isRelationField(field) && field.required) {
                        if (!collectionRecordCreated.includes(field.collection)) {
                            await retryOperation(
                                async () => {
                                    const result = await addRecordPrompt(field.collection, collection.auth)
                ...
                if (!collectionRecordCreated.includes(collectionName)) {
                    await retryOperation(
                        async () => {
                            await addRecordPrompt(collectionName, collection.auth, undefined, relationIds)
                        },
                        [],
                        (error: unknown) => console.log(error),
                        0,
        ...
        await runChildProcess("npx", ["stoker", "delete-tenant"])
        process.exit(1)
    }
}
