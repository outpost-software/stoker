...
export const validateRelationHierarchy = async (
    ...
    const ref = getFirestorePathRef(db, relation.Collection_Path) as CollectionReference
    batchSize.size++
            ...
            if (["OneToOne", "OneToMany"].includes(field.type)) {
                if (field.enforceHierarchy) {
                    if (
                        ...
                        await validateRelationHierarchy(collectionSchema, field, record, transaction, batchSize)
                    }
                }
            }
        }
