...
import { getGlobalConfigModule, getCustomizationFile } from "../initializeStoker.js"
...
export const updateRecord = async (
    ...
    const ref = getFirestorePathRef(db, path)

    context = context || {}
    context.collection = labels.collection

    let originalRecord = await getOne([labels.collection], docId, {
        user: userId,
    ...
    if (enableWriteLog)
        await writeLog("update", "started", partial, path, docId, collectionSchema, undefined, originalRecord)
    ...
    try {
        ...
        await uniqueValidation("update", docId, record, collectionSchema, schema)
    ...
    const preWriteChecks = async (transaction: Transaction, initial?: boolean, batchSize?: { size: number }) => {
        const [latestDeploy, maintenanceMode, latestOriginalRecord, permissionsSnapshot, latestSchema] =
            await Promise.all([
                ...
                getOne([labels.collection], docId, {
                    user: userId,
                    providedTransaction: transaction,
                    noComputedFields: true,
                }),
                userId
                    ? transaction.get(db.collection("system_user_permissions").doc(userId))
        ...
        if (batchSize) batchSize.size += getDocumentRefs(path, docId, schema, currentUserPermissions).length
        ...
        if (updateUserRequired(originalRecord)) {
            const originalPermissionsSnapshot = await transaction.get(
                db.collection("system_user_permissions").doc(originalRecord.User_ID),
            )
    ...
    try {
        ...
        try {
                ...
                async (transaction) => {
                    ...
                    addDenormalized(
                        ...
                        (relationPath: string[], id: string) => {
                            const ref = getFirestorePathRef(db, relationPath)
                            return ref.doc(id)
                        },
            ...
            if (!errorHook?.resolved) {
                ...
                if (deleteUserRequest) {
                    await db.collection(labels.collection).doc(docId).update({ User_ID: FieldValue.delete() })
                }
                throw error
            }
        }
