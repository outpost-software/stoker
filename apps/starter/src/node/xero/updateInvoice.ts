...
export const updateInvoice = (
    ...
    return (async () => {
        ...
        const { xero, tenantId } = await xeroLogin(xeroId, xeroSecret)
        if (!tenantId) {
        ...
        const updateInvoice = async (
            ...
            await xero.accountingApi.updateOrCreateInvoices(tenantId, { invoices: [invoice] }, summarizeErrors, unitdp)
        }
        ...
        const getInvoice = async (company: StokerRecord) => {
            ...
            const response = await xero.accountingApi.getInvoice(tenantId, record.Xero_ID, unitdp)
        ...
        const doc = await db.collection(contactsCollection.labels.collection).doc(contactId).get()
