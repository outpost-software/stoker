...
import { addRecord, deleteRecord, initializeStoker, updateRecord } from "@stoker-platform/node-client"
...
const startStoker = async () => {
    process.env.GCP_PROJECT = tenantName
    dotenv.config({ path: join(process.cwd(), ".env", ".env"), quiet: true })
    dotenv.config({ path: join(process.cwd(), ".env", `.env.${tenantName}`), quiet: true })
...
const getContactId = async () => {
    await startStoker()
    const db = getFirestore()
    const contactSnapshot = await db.collection("Contacts").get()
...
const getWorkOrderId = async () => {
    await startStoker()
    const db = getFirestore()
    const workOrderSnapshot = await db.collection("Work_Orders").get()
...
describe("CLI", async () => {
    ...
    test("add-tenant command creates a tenant", async () => {
        dotenv.config({ path: join(process.cwd(), ".env", ".env"), quiet: true })
        const child = spawn("stoker", ["add-tenant", "--name", tenantName, "--development", "--test-mode"], {
            stdio: ["pipe", "pipe", "pipe"],
        })
    ...
    test("set-tenant command sets tenant", async () => {
        const child = spawn("stoker", ["set-tenant"], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
            },
        })
    ...
    test("add-record-prompt command adds an Area Manager user", async () => {
        const child = spawn("stoker", ["add-record-prompt", "--collection", "Users", "--full-access"], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("get-one command retrieves a record", async () => {
        ...
        const child = spawn("stoker", ["get-one", "-p", `Users/${userId}`], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("get-some command retrieves all records", async () => {
        const child = spawn("stoker", ["get-some", "-p", `Users`], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("delete-record command deletes a record", async () => {
        ...
        const child = spawn("stoker", ["delete-record", "-p", "Users", "-i", userId], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("get-one command as user retrieves a record", async () => {
        ...
        const child = spawn("stoker", ["get-one", "-p", `Users/${userId}`, "-u", adminId], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("get-some command as user retrieves all records", async () => {
        ...
        const child = spawn("stoker", ["get-some", "-p", `Users`, "-u", adminId], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("delete-record command as user deletes a record", async () => {
        ...
        const child = spawn("stoker", ["delete-record", "-p", "Users", "-i", userId, "-u", adminId], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("explain-preload command explains preload", async () => {
        ...
        const child = spawn("stoker", ["explain-preload", "-a", "-i", adminId], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("audit-permissions command audits permissions", async () => {
        const child = spawn("stoker", ["audit-permissions"], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("audit-denormalized command audits denormalized data", async () => {
        const child = spawn("stoker", ["audit-denormalized"], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("audit-relationships command audits relations", async () => {
        const child = spawn("stoker", ["audit-relations"], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
    ...
    test("seed-data command seeds data", async () => {
        const child = spawn("stoker", ["seed-data", "-n", "5", "-r", "5", "-m", "production"], {
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...process.env,
                GCP_PROJECT: tenantName,
...
describe("Cloud Functions", async () => {
    test("validateRelations function validates relations", async () => {
        ...
        const persistedSnapshot = await db.collection("Users").doc(record.id).get()
    ...
    test("autoIncrement function increments record number", async () => {
        ...
        const dependency1 = await db.collection("system_fields").doc("Users").collection("Users-1").doc(userId).get()
        expect(dependency1.data()?.Number).toBe(snapshot.data()?.Number)
        const dependency2 = await db.collection("system_fields").doc("Users").collection("Users-2").doc(userId).get()
    ...
    test("updateIncludeFields function updates include fields", async () => {
        ...
        const snapshot = await db.collection("Users").doc(functionsUserId).get()
    ...
    test("verifyWriteLog function verifies write log", async () => {
        await startStoker()
        const db = getFirestore()
        const snapshot = await db.collection("Users").doc(functionsUserId).collection("system_write_log").get()
...
afterAll(async () => {
    const child = spawn("stoker", ["delete-tenant", "--test-mode"], {
        stdio: ["pipe", "pipe", "pipe"],
        env: {
            ...process.env,
            GCP_PROJECT: tenantName,
