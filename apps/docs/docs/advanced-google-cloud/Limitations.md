...
Currently, if you need to migrate a tenant's data, you'll need to put that tenant into maintenance mode. In future we may add support for zero-downtime migrations.

Additionally, there is no current "down" migration tool for Stoker.
